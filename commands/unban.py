...
if (member.can_restrict_members) or (member.status == "creator") or (member.user.name in config.telegram_admins) or (member.user.name in config.telegram_moders):
	if (len(text) == 2) or (update.message.reply_to_message is not None):
		if update.message.reply_to_message is not None:
			...
			if len(text) == 2:
				update.message.reply_text(f"Слишком много аргументов для разбана по ответу на сообщение.", parse_mode="Markdown")
				target_id = ""
		else:
			target_id = text[1]
			...
			if target_id.isdigit():
				...
				if target_id is not None:
					...
					target_id = ""
		if update.effective_chat.unban_member(target_id):
			update.message.reply_text(f"{target_display_name} разбанен.", parse_mode="Markdown")
		else:
			update.message.reply_text(f"Не удалось разбанить {target_display_name}.", parse_mode="Markdown")
	else:
